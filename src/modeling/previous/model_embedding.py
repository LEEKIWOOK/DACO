...
class CNN_GRU_EMBD(nn.Module):
    def __init__(self, len: int):
        super(CNN_GRU_EMBD, self).__init__()
        ...
        self.embedding_layer = nn.Embedding(
            num_embeddings=5, embedding_dim=self.embd_size, max_norm=True
        )
        self.position_encoding = PositionalEncoding(
            dim=self.embd_size, max_len=len, dropout=0.1
        )

        self.ConvLayer_embd = nn.Sequential(
            nn.Conv1d(128, 128, kernel_size=1, padding="same", stride=1, bias=False),
            nn.BatchNorm1d(128),
            nn.ReLU(),
    ...
    def forward(self, inputs):
        ...
        reg_out = self.embedding_layer(inputs) * math.sqrt(self.embd_size)
        reg_out = self.position_encoding(reg_out)
