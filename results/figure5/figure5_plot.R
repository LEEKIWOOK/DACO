...
f5_data<-rbind(cas9_comp, cas12_comp)
...
for (i in 1:6){
  ...
  tmp <- f5_data %>% filter(Train_data == train_data[i] & Test_data == train_data[i]) 
  stat.test <- tmp %>% wilcox_test(Spearman ~ Model, detailed = T) %>% add_xy_position()
  tmp$Model<-factor(tmp$Model, levels=c(tools[i], tools[7]))
  tmp %>% group_by(Model) %>% summarise(mean = mean(Spearman))
  
  figure_list[[i]] <- ggviolin(tmp, x = "Model", y = "Spearman", fill = "Model", add="boxplot", add.params = list(fill = "white")) + labs(y="Spearman correlation", title = train_data[i]) + 
...
ggsave(plot = g, file=file.path(filename="results/figure5", "figure5.tiff"), scale = 1, width = 8.5, height = 4.5, dpi = 150, units="in", dev="tiff")
ggsave(plot = g, file=file.path(filename="results/figure5", "figure5.png"), scale = 1, width = 8.5, height = 4.5, dpi = 150, units="in", dev="png")
